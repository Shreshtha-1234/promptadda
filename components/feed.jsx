...
const Feed = () => {
  ...
  function searchHelper(searchValue) {
      const filteredPost = original.filter((pst) => {
        return pst.prompt.toLowerCase().includes(searchValue) || pst.creator.username.toLowerCase().includes
        (searchValue) || pst.tag.toLowerCase().includes(searchValue)
    })
    ...
    <div>
      <section className="feed">
        <form className="relative w-full flex-center">
          ...
          value={search}
          onChange={(e)=>{setSearch(e.target.value);searchHelper(e.target.value.toLowerCase()) }} 
